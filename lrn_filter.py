...
def lrn_filter(photometry,fore_gmr,dm=None,flux=False):
    ...
    bluepeak = maxdaycol<0.4
    blued = d[(gl==False) & (c<0.4)]
    #print(maxday,blued)
    if len(blued) == 0:
        redlater = True
        redalways = True
...
if __name__ == '__main__':
    ...
    parser.add_argument("--o",type=str,default='lrn_filters',help="output file to write results in")
    ...
    for ind in range(len(dates)-1):
        ...
        for k in range(len(sources)):
            ...
            if not np.isnan(val_fritz_dms[k]):
                dm_to_use = val_fritz_dms[k].value
                            
            elif not np.isnan(clu_zs[k]):
                if clu_zs[k]<0:
                    dm_to_use = clu_gals[source['id']][0]['dm']
                else:
                    dm_to_use = clu_dms[k].value

            else:
                dm_to_use = fritz_dms[k].value

            if lrn_filter(photometry,fgmr,dm =dm_to_use,flux=True):
                print(source['id'])
                is_sublum, peak_date, peak_absmag = is_subluminous(photometry,dm=dm_to_use,flux=True)
                with open(output,'a') as f:
                    f.write('%s\t%s\t%s\n'%(source['id'],peak_date,peak_absmag))
