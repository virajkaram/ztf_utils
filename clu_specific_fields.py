...
fieldras  = [(30.2, 38.8),(129.0, 141.0),(174.0,186.0),(211.5,223.5),(300,360),(0,60)]
fielddecs = [(-10.25,-3.72),(-2,3),(-3,2),(-2,3),(-1.26,1.26),(-1.26,1.26)]
...
if __name__ == '__main__':
    ...
    parser.add_argument("--o_sub",type=str,default='specific_fields_clu',help="output file to write subluminous results in")
    ...
    for ind in range(len(dates)-1):
        ...
        sources = query_fritz(prog_id,start_date,end_date,args.arx,includePhotometry=False)
        sleep(1)
        ...
        for src in sources:
            for fid in range(len(fielddecs)):
                if (fieldras[fid][0]<src['ra']<fieldras[fid][1]) & (fielddecs[fid][0]<src['dec']<fielddecs[fid][1]):
                    with open(subluminous_output,'a') as f:
                        print('%s\t%.3f\t%.3f\t%s\n'%(src['id'],src['ra'],src['dec'],src['redshift']))
                        f.write('%s\t%.3f\t%.3f\t%s\n'%(src['id'],src['ra'],src['dec'],src['redshift']))
                        continue
