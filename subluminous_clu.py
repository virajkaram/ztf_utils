...
def is_subluminous_clu_kowalski(source,clu_cross_match):
    ...
    if len(gal_dms[gal_matched].value)>0:
        ...
        peak_date = -99
        clu_z = np.nan
    return is_sublum, peak_date, peak_absmag, clu_z, gal_zs[gal_matched]
...
if __name__ == '__main__':
    ...
    parser.add_argument("--o",type=str,default='subluminous_clu',help="output file to write results in")
    ...
    for ind in range(len(dates)-1):
        ...
        for k in range(len(sources)):
            ...
            is_sublum_clu, peak_date_clu, peak_absmag_clu, clu_matched_z, clu_gals_zs_matched = is_subluminous_clu_kowalski(source,clu_cross_match)
