...
def plot_mags(d,c='red',l='ZTF-r',ymn=18,ymx=22):
    snr = np.array(d['forcediffimflux,'],dtype=float)/np.array(d['forcediffimfluxunc,'],dtype=float)
    snt = 3
    snu = 5
    detmask = (snr>snt)
    limmask = np.invert(detmask)

    ddet_mags = -2.5*np.log10(np.array(d[detmask]['forcediffimflux_uJy'],dtype=float)*1e-6/3631)
    ddet_magunc = 1.0857*np.array(d[detmask]['forcediffimfluxunc_uJy'],dtype=float)/np.array(d[detmask]['forcediffimflux_uJy'],dtype=float)
    plt.plot(d[detmask]['jd,']-jd0,ddet_mags,'.',color=c)
    plt.errorbar(d[detmask]['jd,']-jd0,ddet_mags,ddet_magunc,fmt='.',color=c,label=l)#,t['col26']

    dlim_mags = -2.5*np.log10(snu*np.array(d[limmask]['forcediffimfluxunc_uJy'],dtype=float)*1e-6/3631)
...
def plot_mags_atlas(atlaslcfile,ymn='18',ymx='22',jd0=0):
    ...
    plt.plot(c[c_dets]['##MJD']+2400000.5-jd0,-2.5*np.log10(c[c_dets]['uJy']*1e-6/3631),'.',c='cyan',label='ATLAS-c')
    plt.errorbar(c[c_dets]['##MJD']+2400000.5-jd0,-2.5*np.log10(c[c_dets]['uJy']*1e-6/3631),yerr=1.086*c[c_dets]['duJy']/c[c_dets]['uJy'],fmt='.',c='cyan')        
    plt.plot(o[o_dets]['##MJD']+2400000.5-jd0,-2.5*np.log10(o[o_dets]['uJy']*1e-6/3631),'.',c='pink',label='ATLAS-o')
    plt.errorbar(o[o_dets]['##MJD']+2400000.5-jd0,-2.5*np.log10(o[o_dets]['uJy']*1e-6/3631),yerr=1.086*o[o_dets]['duJy']/o[o_dets]['uJy'],fmt='.',c='pink')

    plt.plot(c[c_lims]['##MJD']+2400000.5-jd0,-2.5*np.log10(5*c[c_lims]['uJy']*1e-6/3631),'v',c='cyan',label='ATLAS-o')
    plt.plot(o[o_lims]['##MJD']+2400000.5-jd0,-2.5*np.log10(5*o[o_lims]['uJy']*1e-6/3631),'v',c='pink',label='ATLAS-o')
    plt.ylim(ymx,ymn)
    return plt
...
def binavg(df,binsize):
    ...
    while imax<=n:
        ...
        bflux, bmjd, bsigflux = wavg(df.iloc[imin:imax], 'forcediffimflux_uJy', 'jd,', 'forcediffimfluxunc_uJy')
        bfluxa.append(bflux)
        bmjda.append(bmjd)
...
def wavg_df(group, avg_name='forcediffimflux_uJy', avg2_name='jd,', weight_name='forcediffimfluxunc_uJy'):
    """ http://stackoverflow.com/questions/10951341/pandas-dataframe-aggregate-function-using-multiple-columns
    In rare instance, we may not have weights, so just return the mean. Customize this if your business case
    should return otherwise.
    """
    ...
    w = (group[weight_name])**(-2)
    try:
        return pd.Series({'bflux_uJy':(f * w).sum() / w.sum(), 'bjd':(d * w).sum() / w.sum(), 'bfluxunc_uJy': (w.sum())**(-0.5)})
    except ZeroDivisionError:
        return d.mean(), -1
...
def plot_binned(d, binsize,c='red',l='ZTF-r',ymn=-50,ymx=100,magticks=True):
    ...
    ax1.errorbar(bdjd-jd0,bdf,yerr=bdfunc,fmt='.',c=c,label='%s, %sd'%(l,binsize))
...
def plot_binned_mags_df(d,binsize,c='red',l='ZTF-r',ymn=18,ymx=22,magticks=True, jd0=Time(datetime.utcnow()).jd,write=True):
    bdf = pd.DataFrame.from_records(d,columns=d.colnames)
    ...
    bdf_bin_flux['maglim'] = -2.5*np.log10(bdf_bin_flux['bfluxunc_uJy']*5*1e-6/3631)
    if write:
        bdf_bin_flux.to_csv(f'fps_lcs/binned_{l}.csv')

    bdjd, bdf, bdfunc = bdf_bin_flux['bjd'], bdf_bin_flux['bflux_uJy'], bdf_bin_flux['bfluxunc_uJy']
    snr = bdf/bdfunc
    ...
    ddet_magunc = 1.0857*np.array(bdfunc[detmask],dtype=float)/np.array(bdf[detmask],dtype=float)
    ...
    plt.errorbar(bdjd[detmask]-jd0,ddet_mags,ddet_magunc,fmt='.',color=c,label=l)#,t['col26']

    dlim_mags = -2.5*np.log10(snu*np.array(bdfunc[limmask],dtype=float)*1e-6/3631)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--mag",action='store_true',default=False,help="make only magnitudes as well")
    parser.add_argument("--bin",action='store_true',default=False,help="make only magnitudes as well")
    parser.add_argument("--rbin",type=int,default=1,help="make only magnitudes as well")
    parser.add_argument("--gbin",type=int,default=1,help="make only magnitudes as well")
        ...
        if args.mag:
            if not args.bin:
                ...
                if args.atlas_file is not None:
                    ...
                    plt = plot_mags_atlas(atlaslcfile,ymn=args.ymin,ymx=args.ymax,jd0=jd0)
            ...
            if args.bin:
                plt.figure(figsize=(10,7))
                if len(r)>0:
                    plt = plot_binned_mags_df(r,args.rbin,c='red',l='ZTF-r',magticks=True,ymn=args.ymin,ymx=args.ymax,jd0=jd0)
                if len(i)>0:
                    plt = plot_binned_mags_df(i,args.ibin,c='brown',l='ZTF-i',magticks=True,ymn=args.ymin,ymx=args.ymax,jd0=jd0)
                if len(g)>0:
                    plt = plot_binned_mags_df(g,args.gbin,c='green',l='ZTF-g',magticks=True,ymn=args.ymin,ymx=args.ymax,jd0=jd0)
                #plt.axvline(specjd-jd0,linestyle='--',c='black')
                objname = args.ztf_file.split('.dat')[0]
                plt.legend(fontsize=12)
                plt.xlabel('JD - %i [today]'%(jd0))
    ...
    if args.pgir_file is not None:
        ...
        for key in lc.keys():
            ...
            plt.errorbar(lc[key]['jd']-jd0,lc[key]['psfflux'],lc[key]['psfflux_unc'],fmt='.',c='purple',label='PGIR J')
    ...
    if args.atlas_file is not None:
        ...
        if not args.bin:
            ax1.plot(c['##MJD']+2400000.5-jd0,c['uJy'],'.',c='cyan',label='ATLAS-c')
            ax1.errorbar(c['##MJD']+2400000.5-jd0,c['uJy'],c['duJy'],c='cyan',linestyle='',fmt='.')

            ax1.plot(o['##MJD']+2400000.5-jd0,o['uJy'],'.',c='pink',label='ATLAS-o')
            ax1.errorbar(o['##MJD']+2400000.5-jd0,o['uJy'],o['duJy'],c='pink',linestyle='',fmt='.')
