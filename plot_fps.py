...
def plot_binned_mags_df(d,binsize,c='red',l='ZTF-r',ymn=18,ymx=22,magticks=True,jd0=jd0):
    bdf = pd.DataFrame.from_records(d,columns=d.colnames)
...
if args.ztf_file is not None:
    ...
    if args.mag:
        ...
        if args.bin:
            plt.figure(figsize=(10,7))
            plt = plot_binned_mags_df(r,args.rbin,c='red',l='ZTF-r',magticks=True,ymn=args.ymin,ymx=args.ymax)
            plt = plot_binned_mags_df(i,args.ibin,c='brown',l='ZTF-i',magticks=True,ymn=args.ymin,ymx=args.ymax)
            plt = plot_binned_mags_df(g,args.gbin,c='green',l='ZTF-g',magticks=True,ymn=args.ymin,ymx=args.ymax)
            #plt.axvline(specjd-jd0,linestyle='--',c='black')
            objname = args.ztf_file.split('.dat')[0]
            plt.legend(fontsize=12)
            plt.xlabel('JD - %i [today]'%(jd0))
...
if args.pgir_file is not None:
    ...
    with open(pgirlcfilefile,'r') as json_file:
