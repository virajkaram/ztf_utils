...
def is_subluminous(photometry,dm=None,flux=False):
    ...
    if len(data)==0: #No photometry points. Happened for some sources, example ZTF19acvtmul
        data = np.array([(-99,99)])
        #return True, -99, -99
    #print(np.nanmin(data[:,1])-dm > -16, data[:,0][np.nanargmin(data[:,1])], np.nanmin(data[:,1])-dm)
...
def is_subluminous_clu_kowalski(source,clu_cross_match):
    srcname = source['id']
    ...
    print(gal_zs[gal_matched],gal_dms[gal_matched].value)
    
    if len(gal_dms[gal_matched].value)>0:
        is_sublums, peak_date, peak_absmags = is_subluminous(photometry,dm = gal_dms[gal_matched].value,flux=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--start_date",type=float,default=57800,help="start date in MJD")
    ...
    parser.add_argument("--arx",action='store_false')
    ...
    subluminous_output = args.o_sub + '_' + prog_id + '_' + str(int(args.start_date)) + '_' + str(int(args.end_date)) + '.dat'
    summary_filename = subluminous_output.replace('.dat','_summary.dat')
    ...
    with open(summary_filename,'w') as sum_f:
        sum_f.write('Run with command : python subluminous_red_lrn_filter_archival.py --start_data %s --end_date %s --nslices %s --prog_id %s --o_sub %s --o_lrn %s\n'%(args.start_date,args.end_date,args.nslices,args.prog_id,args.o_sub,args.o_lrn))
    ...
    for ind in range(len(dates)-1):
        ...
        #num_sources = query_fritz(prog_id,start_date,end_date,args.arx,includePhotometry=False)

        #if len(num_sources) == 0:
        #    with open('summary_subluminous_arx.dat','a') as sum_f:
        #        sum_f.write('Queried %s sources last detected between between %s and %s\n'%(len(num_sources), start_date, end_date))
        #    continue
            
        sources, response_status = query_fritz(prog_id,start_date,end_date,args.arx)
        total_sources = total_sources + len(sources)

        if response_status<0:
            with open(summary_filename,'a') as sum_f:
                sum_f.write('QUERY FAILED between %s and %s\n.'%(start_date, end_date))
        ...
        if args.arx:
            with open(summary_filename,'a') as sum_f:
                sum_f.write('Queried %s sources last saved between between %s and %s\n'%(len(sources), start_date, end_date))
                sum_f.write('Total sources queried so far : %s\n'%(total_sources))
